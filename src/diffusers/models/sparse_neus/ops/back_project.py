...
def back_project_sparse_type(coords, origin, voxel_size, feats, KRcam, sizeH=None, sizeW=None, only_mask=False,
    ...
    for batch in range(bs):
        ...
        feats_batch = feats[:, batch] # (8, 1, c, h, w) -> (8, c, h, w)
        proj_batch = KRcam[:, batch] # (8, 1, 4, 4) -> (8, 4, 4)

        grid_batch = coords_batch * voxel_size + origin_batch.float()
        # print("===saving grid batch in get condition volume====")
        # import numpy as np
        # np.save('debug/grid_batch.npy',  grid_batch.detach().cpu().numpy())
        if debug:
            ray_points = np.load('debug/points.npy')
            grid_batch = torch.from_numpy(ray_points).to(feats.dtype).to(device) # (-1, 3)
        rs_grid = grid_batch.unsqueeze(0).expand(n_views, -1, -1)
        rs_grid = rs_grid.permute(0, 2, 1).contiguous() # (8, 3, -1)
        nV = rs_grid.shape[-1]
        rs_grid = torch.cat([rs_grid, torch.ones([n_views, 1, nV]).to(device)], dim=1) # (8, 4, -1)
        ...
        if debug:
            ...
            features = features.view(n_views, c, 64, 64, 48)[...,0]
            from torchvision.utils import save_image
            save_image(feats_batch, 'debug/clean_img.png', nrow=4)
            save_image(features, 'debug/debug_img.png', nrow=4)
