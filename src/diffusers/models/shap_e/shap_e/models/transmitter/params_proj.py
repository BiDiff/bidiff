...
class ChannelsProj(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        h = torch.einsum("bvd,vcd->bvc", x_bvd, w_vcd) # bvcd, bvcd -> bvc
        if self.use_ln:
            h = self.norm(h)
...
class ChannelsParamsProj(ParamsProj):
    def __init__(
        ...
        *,
        ...
        self.learned_scale = learned_scale # 0.0625
        self.use_ln = use_ln # True
        for k, (vectors, channels) in self.flat_shapes.items(): # (256, 93), (256, 256)
