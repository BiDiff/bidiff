...
class StableDiffusionControlNetPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    ...
    def _encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            # add_special_tokens=clean_caption
            ...
            if (hasattr(self.text_encoder.config, "use_attention_mask") and \
                self.text_encoder.config.use_attention_mask) or args.text_encoder_use_attention_mask:
                attention_mask = text_inputs.attention_mask.to(device)
            else:
                attention_mask = None
                raise NotImplementedError

            # print('\nDEBUG 00:', text_input_ids, attention_mask)
            prompt_embeds = self.text_encoder(
                text_input_ids.to(device),
                attention_mask=attention_mask,
            )
            prompt_embeds = prompt_embeds[0]
            # print('\nDEBUG 01:', prompt_embeds, prompt_embeds_)
