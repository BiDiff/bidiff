...
def diffusion_from_config(config: Union[str, Dict[str, Any]]) -> "GaussianDiffusion":
    ...
    schedule = config["schedule"] # exp
    steps = config["timesteps"] # 1024
    respace = config.get("respacing", None) # None
    mean_type = config.get("mean_type", "epsilon") # x_start
    ...
    kwargs = dict(
        betas=betas,
        model_mean_type=mean_type, # x_start
...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        elif self.model_mean_type in ["x_start", "epsilon"]: # x_strat
    ...
    def p_sample(
        ...
        # print("=====dtype====", sample.dtype, out["pred_xstart"].dtype)
    ...
    def p_sample_loop(
        ...
        for sample in self.p_sample_loop_progressive(
            ...
            final = sample
        return final["sample"]
        # NOTE(lihe): use pred x0 to debug !!!!
    ...
    def p_sample_loop_progressive(
        ...
        for i in indices:
            ...
            with th.no_grad():
                ...
                img = out["sample"]
                # NOTE(lihe): save latents for later visulaization
                if i % 50 == 0:
                    print("========", i)
                    # pred_x0 = self.unscale_out_dict(out)['pred_xstart']
                    # np.save('debug_data/pred_x0_{}.npy'.format(i), pred_x0.detach().cpu().numpy())
                    pred_xt = self.unscale_out_dict(out)['sample']
                    np.save('debug_data/sample_{}.npy'.format(i), pred_xt.detach().cpu().numpy())
