...
from diffusers.models.diff3d.utils import get_voxel_coordinates, VIT_PATCH_NUMBER, VIEW_IMAGE_RES
...
class CrossAttention(nn.Module):
    def __init__(self, feature_dim, sketch_dim: int, kernel_size: float = 1.0,
        ...
        self.image_size = image_size # 3d voxel dim
    ...
    def get_attn_mask(self, pixels, vit_pixels, kernel_size=None, res=VIEW_IMAGE_RES):
        ...
        #            self.vit_res/2)
        y_index = ((2 * pixels[:, :, 0:1] + 1)/res *
                   self.vit_res/2) # NOTE(lihe): no need to reverse?
    ...
    def forward(self, x, sketch_feature, projection_matrix=None, kernel_size=None, ref_img=None):
        q = self.q(x).reshape(x.shape[0], self.feature_dim, -1).transpose(1,
                                                                          2) + self.voxel_pe.to(x.device).unsqueeze(0) # [b, l, c]
        if self.vit_local and not self.vit_global:
            if projection_matrix is not None:
                voxel_points = torch.from_numpy(get_voxel_coordinates(
                    ...
                    resolution=x.shape[-1], size=0.35)).unsqueeze(2).to(projection_matrix.device).float()
                pc_in_camera = projection_matrix @ voxel_points # [b, 1, 4,4] @ [-1, 4, 1] -> [b, 1, 4, 1]
                pixels = pc_in_camera[:, :, 0:2]/pc_in_camera[:, :, 2:3]

                pixels = clamp_pixel(pixels).squeeze(3)

                # print("===pixels====", pixels.shape)
                # debug_mask = torch.zeros(voxel_points.shape[0])
                # total_voxels = voxel_points.shape[0]
                # print("===total voxels===", total_voxels)
                # from torchvision.utils import save_image
                # save_image(ref_img, 'debug/test_occ2.png')
                # import tqdm
                # for i in tqdm.tqdm(range(total_voxels)):
                #     x = pixels[0, i, 0].long()
                #     y = pixels[0, i, 1].long()
                #     # rgb = ref_img[0, :, y, x] # b, 3, h, w
                #     rgb = ref_img[0, :, y, x] # b, 3, h, w
                #     # rgb = ref_img[0, :, x, y] # b, 3, h, w
                #     if rgb.sum() > 1e-5:
                #         debug_mask[i] = 1
                # np.save('debug/test_occ_voxel_points.npy', voxel_points.detach().cpu().numpy())
                # np.save('debug/test_occ_pixels.npy', pixels.detach().cpu().numpy())
                # np.save('debug/test_occ_mask.npy', debug_mask.numpy())
                # print("======self.vit_res:", self.vit_res)
                # exit()
                ...
                # debug_attn_mask = attn_mask == 0
                # debug_attn_mask = debug_attn_mask[0][debug_mask > 0] # [N', 64*64]
                # # debug_attn_mask = attn_mask[0][debug_mask > 0] # [N, 64*64] -> [N', 64*64]
                # debug_attn_mask = debug_attn_mask.sum(0) > 0
                # debug_attn_mask = debug_attn_mask.view(1, 1, 64, 64).float()
                # # debug_attn_mask = debug_attn_mask[0].view(1, 1, 64, 64).float()
                # save_image(debug_attn_mask, 'debug/test_occ2_mask.png')
                # exit()
            ...
            if projection_matrix is not None:
                voxel_points = torch.from_numpy(get_voxel_coordinates(
                    resolution=x.shape[-1], size=1)).unsqueeze(2).to(projection_matrix.device).float()
...
def clamp_pixel(pixels):
    ...
    return torch.clip(torch.round(pixels), 0, 63)
