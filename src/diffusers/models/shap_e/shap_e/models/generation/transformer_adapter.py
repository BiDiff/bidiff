...
class MultiheadAttention(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.attention = QKVMultiheadAttention(device=device, dtype=dtype, heads=heads, n_ctx=n_ctx, use_adapter=use_adapter)
        self.use_adapter = use_adapter
        init_linear(self.c_qkv, init_scale)
        init_linear(self.c_proj, init_scale)
        if self.use_adapter:
            self.gate = torch.nn.Parameter(torch.zeros(1, self.heads, 1, 1))
            # self.gate = torch.nn.Parameter(torch.ones(1, self.heads, 1, 1))
            # self.gate = torch.nn.Parameter(torch.ones(1))

    def forward(self, x, adapter=None):
        if not self.use_adapter:
            x = self.c_qkv(x)
            x = checkpoint(self.attention, (x, None, None), (), True)
            # x = checkpoint(self.attention, (x, None, None), (), False) 
            x = self.c_proj(x)
        else:
            x = self.c_qkv(x)
            assert adapter is not None, "adapter is None"
            adapter = self.c_qkv(adapter)
            x = checkpoint(self.attention, (x, adapter, self.gate), (), True)
            # x = checkpoint(self.attention, (x, adapter, self.gate), (), False)
            # x = checkpoint(self.attention, (x, adapter), (), False)
            x = self.c_proj(x)
        return x
...
class QKVMultiheadAttention(nn.Module):
    def __init__(self, *, device: torch.device, dtype: torch.dtype, heads: int, n_ctx: int, use_adapter: bool=False):
        super().__init__()
        self.device = device
        self.dtype = dtype
        self.heads = heads
    ...
    def forward(self, qkv, adapter, gate):
        ...
        # NOTE(lihe): add adapter
        if self.use_adapter:
            adapter_len = adapter.shape[1]
            adapter = adapter.view(bs, adapter_len, self.heads, -1)
            _, adapter_k, adapter_v = torch.split(adapter, attn_ch, dim=-1) # [bshc]

        weight = torch.einsum(
            "bthc,bshc->bhts", q * scale, k * scale
        )  # More stable with f16 than dividing afterwards
        wdtype = weight.dtype
        weight = torch.softmax(weight.float(), dim=-1).type(wdtype) # weight: (bhts) value: (bshc) --> *(b,h,t,c) --> (b,t,h,c)

        output = torch.einsum("bhts,bshc->bthc", weight, v).reshape(bs, n_ctx, -1)
        if self.use_adapter:
            adapter_scores = torch.einsum(
                "bthc,bshc->bhts", q * scale, adapter_k * scale
            )
            adapter_scores = gate * torch.softmax(adapter_scores.float(), dim=-1).type(wdtype)
            residual = torch.einsum("bhts,bshc->bthc", adapter_scores, adapter_v)
            output = output + torch.einsum("bhts,bshc->bthc", adapter_scores, adapter_v).reshape(bs, n_ctx, -1)
...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, adapter: Optional[torch.Tensor]=None):
        x = x + self.attn(self.ln_1(x), adapter=adapter)
...
class Transformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.adapt_flags = [((layers - i) <= adapter_layer,  layers - i - 1) for i in range(layers)]
        self.resblocks = nn.ModuleList(
            [
                ResidualAttentionBlock(
                    ...
                    heads=heads,
                    init_scale=init_scale,
                    use_adapter= self.adapt_flags[i][0],
                )
    ...
    def forward(self, x: torch.Tensor, adapter: Optional[torch.Tensor]=None):
        for i, block in enumerate(self.resblocks):
            flag, index = self.adapt_flags[i]
            if flag:
                adapt_input = adapter[index]
            else:
                adapt_input = None
            x = block(x, adapt_input)
        return x
...
class PointDiffusionTransformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        layers: int = 12, # 24
        heads: int = 8, # 16
        init_scale: float = 0.25, # 0.25
        time_token_cond: bool = False,
        use_pos_emb: bool = False,
        pos_emb_init_scale: float = 1.0, # 0.05
        pos_emb_n_ctx: Optional[int] = None, # 1024
        adapter_layer: int = 12,
        adapter_in_ch: int = 768,
    ):
        super().__init__()
        self.input_channels = input_channels # 1024
        self.output_channels = output_channels # 2048
        self.n_ctx = n_ctx # 1025
        self.time_token_cond = time_token_cond # True
        self.use_pos_emb = use_pos_emb # True
        self.time_embed = MLP(
    ...
    def _forward_with_cond(
        self, x: torch.Tensor, cond_as_token: List[Tuple[torch.Tensor, bool]], adapter: Optional[torch.Tensor]=None,
    ) -> torch.Tensor:
        ...
        if adapter is not None:
            adapter = self.ln_pre(adapter)
        h = self.backbone(h, adapter)
        h = self.ln_post(h)
        if len(extra_tokens):
...
class CLIPImagePointDiffusionTransformer(PointDiffusionTransformer):
    def __init__(
        ...
        *,
        ...
        adapter_layer: int = 0,
        adapter_len: int = 0,
        cond_drop_prob_adapter: float = 0.0,
        adapter_in_ch: int = 768,
        **kwargs,
    ):
        super().__init__(
            device=device, dtype=dtype, n_ctx=n_ctx + int(token_cond), pos_emb_n_ctx=n_ctx, 
            adapter_layer=adapter_layer, adapter_in_ch=adapter_in_ch, **kwargs
        )
        self.n_ctx = n_ctx # 1024
        self.token_cond = token_cond # True
        self.clip = (FrozenImageCLIP if frozen_clip else ImageCLIP)(device) # frozen clip
        self.clip_embed = nn.Linear(
            self.clip.feature_dim, self.backbone.width, device=device, dtype=dtype
        )
        self.cond_drop_prob = cond_drop_prob # 0.1
        self.cond_drop_prob_adapter = cond_drop_prob_adapter
        self.adapter_layer = adapter_layer
        self.adapter_len = adapter_len
        if adapter_len > 0:
            self.adapter_query = nn.Embedding(adapter_len * adapter_layer, self.backbone.width)
            self.adapter_in_proj = nn.Linear(
                adapter_in_ch, self.backbone.width, device=device, dtype=dtype
            )
    ...
    def forward(
        ...
        :param adapter: controlnet condition [N x L x C]
        :return: an [N x C' x T] tensor.
        """
        assert x.shape[-1] == self.n_ctx
        ...
        # if self.training:
        #     mask = torch.rand(size=[len(x)]) >= self.cond_drop_prob_adapter
        #     adapter = adapter * mask[:, None, None].to(adapter) # N, C', L

        # NOTE(lihe): fuse embedding adapter and clip tokens
        if self.adapter_layer > 0:
            adapter = self.adapter_in_proj(adapter).unsqueeze(0) # N x C' x L -> 1 x N x L X C, L=8
            prompt = self.adapter_query.weight.reshape(
                self.adapter_layer, self.adapter_len, -1
            ).unsqueeze(1) # 12 x 1 x L x 1024
            adapter = adapter + prompt # 12 x N x 8 x 1024
        ...
        return self._forward_with_cond(x, cond, adapter)
