    ...
    def forward(
        ...
        for attn, resnet in zip(self.attentions, self.resnets[1:]):
            if self.use_view_attn:
                hidden_states = rearrange(hidden_states, '(b v) c h w -> b c (v h) w', v=self.num_views)
                encoder_hidden_states_ = rearrange(encoder_hidden_states, '(b v) l c -> b v l c', v=self.num_views)[:, 0]
            else:
            ...
            if self.use_view_attn:
                hidden_states = rearrange(hidden_states, 'b c (v h) w -> (b v) c h w', v=self.num_views)
            hidden_states = resnet(hidden_states, temb)
...
class UNetMidBlock2DSimpleCrossAttn(nn.Module):
    ...
    def forward(
        ...
        for attn, resnet in zip(self.attentions, self.resnets[1:]):
            # attn
            if self.use_view_attn:
                assert mask is None or attention_mask is not None
                hidden_states = rearrange(hidden_states, '(b v) c h w -> b c (v h) w', v=self.num_views)
                encoder_hidden_states_ = rearrange(encoder_hidden_states, '(b v) l c -> b v l c', v=self.num_views)[:, 0]
            else:
                encoder_hidden_states_ = encoder_hidden_states
        ...
        for resnet, attn in zip(self.resnets, self.attentions):
            if self.training and self.gradient_checkpointing:
                ...
                if self.use_view_attn:
                    ...
                    encoder_hidden_states_ = encoder_hidden_states
                hidden_states = torch.utils.checkpoint.checkpoint(
                    create_custom_forward(attn, return_dict=None),
                    hidden_states,
                    encoder_hidden_states_,
                    mask,
                    use_reentrant=False,
