...
from diffusers.models.sparse_neus.sparse_cost_regnet import SparseCostRegNet, SparseCostRegNetV2
from diffusers.models.sparse_neus.torchsparse_utils import sparse_to_dense_channel
from diffusers.models.sparse_neus.ops.grid_sampler import grid_sample_3d, tricubic_sample_3d

from diffusers.models.sparse_neus.ops.back_project import back_project_sparse_type
from diffusers.models.sparse_neus.ops.generate_grids import generate_grid

from inplace_abn import InPlaceABN

from diffusers.models.sparse_neus.embedder import Embedding
...
class LatentSDFLayer(nn.Module):
    ...
    def forward(self, inputs, latent):
        ...
        x = inputs
        # for l in range(0, self.num_layers - 1): # NOTE(lihe): fix bug here !
        for l in range(0, self.num_layers):
...
class LatentVolumeNetwork(nn.Module):
    '''
    Coarse-to-fine sparse cost regularization network
    return sparse volume feature for extracting sdf and def
    '''

    def __init__(self, ch_in, voxel_size, vol_dims,
        ...
        self.new_sdf_arc = new_sdf_arc
        if new_sdf_arc:
            from diffusers.models.sparse_neus.pos_encoder import PositionalEncoding
            self.code_3d = PositionalEncoding(num_freqs=6, d_in=2, freq_factor=1.5, include_input=True)
            print("=========== Using pos enc for 3d prior ==========")
            cond_dim = self.code_3d.d_out + 3
        else:
            cond_dim = 5

        if use_3d_prior:
            sparse_ch_in = sparse_ch_in + cond_dim
        ...
        if add_temb:
            ...
            self.sparse_costreg_net = SparseCostRegNetV2(
                d_in=sparse_ch_in, d_out=self.regnet_d_out, temb_channels=temb_channels)
            # self.emb_proj =  nn.Linear(temb_channels, 16)
        else:
            self.sparse_costreg_net = SparseCostRegNet(
                d_in=sparse_ch_in, d_out=self.regnet_d_out) 
        ...
        self.sdf_def_layer = LatentSDFLayer(d_in=3,
                                        # d_out=self.hidden_dim + 1 + 3,
                                        d_out=1 + 3,
                                        ...
                                        # d_conditional_feature=16 if not self.add_temb else 32,  # self.regnet_d_out
                                        d_conditional_feature=16
                                        )
        assert self.iso_surface in ['dmtet', 'flexicubes']
        if self.iso_surface == 'flexicubes':
            self.weight_layer = LatentSDFLayer(d_in=4 * 8,
                                        d_out=21,
                                        d_hidden=self.hidden_dim,
                                        n_layers=num_sdf_layers,
                                        multires=multires,
                                        geometric_init=False, # True,
                                        weight_norm=False, # True,
                                        activation=activation,
                                        # d_conditional_feature=16 if not self.add_temb else 32,  # self.regnet_d_out
                                        d_conditional_feature=16 * 8
                                        )
        
        self.voxel_cond = voxel_cond
        if self.voxel_cond:
            self.voxel_embed = nn.Linear(3, 16)
            self.voxel_extractor =  SparseCostRegNet(
                d_in=16, d_out=16)
    ...
    def get_conditional_volume(self, feature_maps, partial_vol_origin, proj_mats, sizeH=None, sizeW=None, lod=0,
        ...
        # inject 3d prior
        if noisy_latents_3d is not None:
            assert bs == 1, "only support bs=1 when using 3d prior."
            ...
            coords_batch = up_coords[:, 1:] # bs=0
            coords_batch = coords_batch.view(-1, 3)
            origin_batch = partial_vol_origin[0].unsqueeze(0)
            grid_batch = coords_batch * self.voxel_size + origin_batch.float()
            
            # NOTE(lihe): scale back to [-1, 1]
            grid_batch = grid_batch / 0.35 #(N, 3)
            grid_batch = grid_batch.view(1, -1, 1, 3) # (b, N, 1, 3)
            query = Query(
                position=grid_batch,
                direction=None,
            )
            ...
            # NOTE(lihe): new sdf arc, do positional encoding
            if self.new_sdf_arc:
                feats_3d = self.code_3d(torch.cat([density, sdf], dim=-1).view(-1, 2))
                feats_3d = feats_3d.view(*density.shape[:-1], -1)
                feats_3d = torch.cat([feats_3d, color], dim=-1) # [1, N, 1, 24 + 3]
            else:
                feats_3d = torch.cat([density, sdf, color], dim=-1) # [1, N, 1, 5]
            
            feats_3d = feats_3d.view(-1, feats_3d.shape[-1]) # [N, 5]

            # NOTE(lihe): drop the shap-e information at some training steps
            # drop_prob = 0.1
            drop_prob = 0.0 # NO DROP
            if drop_prob > 0. and random.random() < drop_prob:
                feats_3d = feats_3d.new_zeros(*feats_3d.shape)
            
            feat = torch.cat([feat, feats_3d], dim=-1)
        ...
        if self.voxel_cond:
            assert voxels is not None # [b, N, 3]
            assert bs == 1
            scale = 0.7
            voxel_size = 0.05
            device = voxels.device
            voxel_feats = self.voxel_embed(voxels).view(-1, 16)
            vol_dims = torch.tensor([scale, scale, scale]) / voxel_size + 1
            vol_dims = vol_dims.int()
            # voxelization
            origins = torch.tensor([-scale/2., -scale/2., -scale/2.]).to(device)
            coords = ((voxels - origins) / voxel_size).int()
            assert (coords >= 0).all()
            assert (coords < vol_dims[0]).all()
            voxel_coords = torch.cat([coords, torch.ones(coords.shape[0], 1).to(device) * 0], dim=-1)
            voxel_feats = SparseTensor(voxel_feats, voxel_coords.to(
                                        torch.int32))
            voxel_feats = self.voxel_extractor(voxel_feats)
            voxel_dense_volume, voxel_valid_mask_volume = self.sparse_to_dense_volume(voxel_coords[:, :-1], voxel_feats, vol_dims, interval,
            ...
            pts = grid_batch
            N_q = pts.shape[0]
            pts = pts.view(bs, 1, 1, N_q, 3) # - should be in range (-1, 1)
            pts = pts / 0.35 # scale to -1 ~ 1
            pts = torch.flip(pts, dims=[-1])
            sampled_feature = grid_sample_3d(voxel_dense_volume, pts)  # [1, c, 1, 1, num_pts]
            sampled_feature = sampled_feature.view(-1, N_q).to(device) # bs, C, N_q
            sampled_feature = sampled_feature.permute(1,0).contiguous()
            feat = torch.cat([feat, sampled_feature], dim=-1)
        ...
        feat = self.sparse_costreg_net(sparse_feat, emb=emb)
    ...
    def get_sdf_def_prediction(self, pts, conditional_volume, split_batch=None, flexicubes_indices=None):
        dtype = pts.dtype
        with torch.autocast('cuda', enabled=False):
            conditional_volume = conditional_volume.float()
            pts = pts.float()
            num_pts = pts.shape[0]
            device = pts.device
            pts_ = pts.clone()
            pts = pts.view(1, 1, 1, num_pts, 3)  # - should be in range (-1, 1)
            # NOTE(lihe): fix normalization bug, [-0.35, +0.35] --> [-1, 1]
            pts = pts / 0.35

            pts = torch.flip(pts, dims=[-1])

            # sampled_feature = grid_sample_3d(conditional_volume, pts)  # [1, c, 1, 1, num_pts]
            sampled_feature = grid_sample_3d(conditional_volume, pts.detach())  # [1, c, 1, 1, num_pts] # NOTE:!!!!!
            sampled_feature = sampled_feature.view(-1, num_pts).permute(1, 0).contiguous().to(device) # (num_pts, c)
            
            if split_batch is None:
                sdf_def_pts = self.sdf_def_layer(pts_, sampled_feature)
                if self.iso_surface == 'flexicubes':
                    ##
                    final_feat = sampled_feature.unsqueeze(0) # [1, num_pts, c]
                    ...
                    ##
                    sdf_features_pts = sdf_def_pts.detach() # 64
                    with torch.no_grad():
                        sdf_features_pts = sdf_features_pts.unsqueeze(0)
                        cond_feat = torch.index_select(input=sdf_features_pts, index=flexicubes_indices.reshape(-1), dim=1)
                        cond_feat = cond_feat.reshape(sdf_features_pts.shape[0], flexicubes_indices.shape[0], flexicubes_indices.shape[1] * sdf_features_pts.shape[-1])
                    ##
                    weights_pts = self.weight_layer(cond_feat[0], grid_feat[0])
                    weights_pts = weights_pts * 0.1
            else:
                raise NotImplementedError

        sdf_pts = sdf_def_pts.to(dtype)
        sampled_feature = sampled_feature.to(dtype)
        outputs = {}
        outputs['sdf_pts'] = sdf_pts[:, :1]
        outputs['def_pts'] = sdf_pts[:, 1:4]
        outputs['sdf_features_pts'] = sdf_pts[:, 4:]
        outputs['sampled_latent'] = sampled_feature
        if self.iso_surface == 'flexicubes':
            weights_pts = weights_pts.to(dtype)
            outputs['weights_pts'] = weights_pts

        return outputs

    def sdf(self, pts, conditional_volume, lod, split_batch=None, emb=None):
        dtype = pts.dtype
        with torch.autocast('cuda', enabled=False):
            conditional_volume = conditional_volume.float()
            pts = pts.float()
            ...
            # timestep embedding
            # NOTE(lihe): we remove the temb to debug
            # if emb is not None and self.add_temb:
            #     emb = self.emb_proj(emb).view(1, -1).repeat(sampled_feature.shape[0], 1) # [1, 16]
            #     sampled_feature = torch.cat([sampled_feature, emb], dim=1)
            if split_batch is None:
                sdf_pts = self.sdf_layer(pts_, sampled_feature)
            else:
                sdf_pts_list = []
                for i in range(math.ceil(num_pts / split_batch)):
                    # print("======for loop sdf====", i)
                    pdb.set_trace()
                    sdf_pts = self.sdf_layer(pts_[i*split_batch : (i+1)*split_batch], sampled_feature[i*split_batch : (i+1)*split_batch])
                    sdf_pts_list.append(sdf_pts)
                sdf_pts = torch.cat(sdf_pts_list, dim=0)
                assert sdf_pts.shape[0] == num_pts

        sdf_pts = sdf_pts.to(dtype)
        sampled_feature = sampled_feature.to(dtype)
        outputs = {}
    ...
    @torch.enable_grad()
    def gradient(self, x, conditional_volume, lod, emb=None):
        """
        ...
        output = self.sdf(x, conditional_volume, lod, emb=emb)
