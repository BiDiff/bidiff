...
def betas_for_alpha_bar_shap_e(num_diffusion_timesteps, alpha_bar, max_beta=0.999):
    """
...
class DDPMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def step_w_device(
        ...
        t = timestep
        if isinstance(t, int):
            t = torch.tensor([t] * model_output.shape[0], device=device)
        prev_t = t + 1
        for i, t_ in enumerate(t):
            prev_t_ = self.previous_timestep(t_.item())
            prev_t[i] = torch.tensor(prev_t_).to(device)
        # prev_t = self.previous_timestep(t)
        ...
        # alpha_prod_t_prev = self.alphas_cumprod.to(device)[prev_t] if prev_t >= 0 else self.one.to(device)
        alpha_prod_t_prev = self.alphas_cumprod.to(device)[prev_t]
        alpha_prod_t_prev[prev_t < 0] = 1.
    ...
    def add_noise_x0_custom_t(
        ...
        if t > 0:
            ...
            variance = (self._get_variance(t, predicted_variance=None, variance_type="fixed_small") ** 0.5) * variance_noise
